...
require("naughty")
require("menubar")
-- Revelation : Mac OS X Expose like
require("revelation")


-- launch Cairo Composite Manager (use 4% of CPU without solicit visual effects ... :/)
-- used for transparency and visual effects
...
tags = {
  names = {"[1:main]", "[2:www]", "[3:term]", "[4:chat]", "[5:work]", "[6:foo]"},
...
vicious.register(oswidget, vicious.widgets.os, "  Kernel : $2 | ")

cpuwidget = wibox.widget.textbox()
vicious.register(cpuwidget, vicious.widgets.cpu, "CPU : $1% ", 10)

cputempwidget = wibox.widget.textbox() 
vicious.register(cputempwidget, vicious.widgets.thermal, "$1° | ", 10, "thermal_zone0")

memwidget = wibox.widget.textbox()
vicious.register(memwidget, vicious.widgets.mem, "MEM : $1% ($2MB/$3MB) | ", 10)

fswidget = wibox.widget.textbox()
vicious.register(fswidget, vicious.widgets.fs, 
  function (widget, args)
    return "HDD : " .. args["{/ used_gb}"] .. "/" .. args["{/ size_gb}"] .. "GB | "
  end, 60)

batwidget = wibox.widget.textbox()
-- vicious.register(batwidget, vicious.widgets.bat, "BAT : $2% ($1 $3 remaining) | ", 60, "BAT1")
vicious.register(batwidget, vicious.widgets.bat, 
  function (widget, args)
    if args[3] == "N/A" then
      return "BAT : " .. args[2] .. "% (" .. args[1] ..") | "
    else
      return "BAT : " .. args[2] .. "% (" .. args[1] .. " " .. args[3] .. " remaining) | "
    end
  end, 60, "BAT1")
...
vicious.register(upwidget, vicious.widgets.uptime, 
  function (widget, args)
    if args[1] == 0 then
      return "Uptime : " .. args[2] .. ":" .. args[3] .. " | "
    else
      return "Uptime : " .. args[1] .. "d " .. args[2] .. ":" .. args[3] .. " | "
    end
  end, 60)
...
vicious.register(ethwidget, vicious.widgets.net,
  function (widget, args)
      return "ETH0 UP : " .. args["{eth0 up_mb}"] .. "MB / DOWN : " .. args["{eth0 down_mb}"] .. "MB"
  end, 10)
...
for s = 1, screen.count() do
    ...
    mid_layout:add(oswidget)
    mid_layout:add(pkgwidget)
    mid_layout:add(upwidget)
--    mid_layout:add(cpuicon)
    mid_layout:add(cpuwidget)
    mid_layout:add(cputempwidget)
    mid_layout:add(memwidget)
    mid_layout:add(fswidget)
    mid_layout:add(batwidget)
    mid_layout:add(ethwidget)
    mid_layout:add(wifiwidget)
